...
- **Stable equilibrium.** For a given level of somatic maintenance investment and activity, an agent reaches a steady-state wear level where repair balances accumulation. At equilibrium, effective traits are degraded but still functional.
...
- **Trait-space-derived lifespan.** An agent's position on the survive-vs-reproduce axis determines its lifespan. High somatic maintenance investment produces long-lived agents with low equilibrium wear. Low investment produces short-lived agents with high equilibrium wear. The lifespan gradient is continuous, not binary.
...
**Energy income** enters reserve through acquisition flows: photosynthesis (flow 1) or consumption (flow 3, which covers both living targets and carcasses). Each flow's yield depends on the agent's trait investment, the availability of the resource it targets, and an amplifying factor external to the trait budget.
...
**2. Sessile vs. mobile.** Sessile and mobile strategies face fundamentally different resource access. This divide emerges from two constraints: the trait budget (investing in mobility leaves less for photosynthetic absorption and nutrient absorption) and contact-time gating (sustained substrate contact is required for both nutrient uptake and spore dispersal, so mobile agents forfeit both). No explicit gate prevents mobile agents from photosynthesizing — the economics of the budget and the nutrient bottleneck make it unviable.
...
**5. Specialist vs. generalist.** The trait budget makes this trade-off inescapable: investing in multiple acquisition strategies means lower investment in each. A specialist concentrating its budget in one capability achieves higher performance in that niche than a generalist spreading the same budget across many. The generalist can exploit multiple niches but is outcompeted by any specialist within the specialist's niche. This is a mathematical property of the L1 budget constraint, not a parameter to be tuned.

**6. Sense vs. save.** Wider sensing range enables better decisions but consumes trait budget and costs energy. Agents must balance the value of information against its price in both budget allocation and metabolic cost.
