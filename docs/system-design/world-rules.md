...
**9. Growth.** Living agent (reserve) → Living agent (structure). When an agent's reserve income exceeds its metabolic costs, the resulting surplus is split immediately by **kappa**, the agent's heritable soma-vs-reproduction allocation trait. This is DEB-style flow allocation: kappa partitions the *flow* of surplus at the moment it is generated, not the *stock* of reserve at the moment of spending. The kappa fraction is committed to soma (spent that tick on somatic repair and on growth — converting reserve to structure, lossy, with a fraction dissipating to heat per flow 7). The (1 − kappa) fraction is committed to the agent's **reproductive allocation** — an earmarked sub-account of reserve that accumulates across ticks until a reproductive event fires (flow 4). Growth itself is not a decision — it is an automatic consequence of being well-fed. Structure starts at zero for newborn agents and accumulates over the agent's lifetime. The growth rate depends on the soma share of available surplus after metabolism is paid.
