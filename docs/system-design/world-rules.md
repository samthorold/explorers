...
**2. Nutrient uptake.** Available pool → Living agent. Agents extract nutrient from the available pool at their location. The uptake rate depends on the agent's autotrophy investment — the same sessile infrastructure (analogous to roots) that captures light also extracts nutrients from the substrate. Co-located agents share the available pool proportionally, weighted by their effective uptake rate. Nutrient uptake is the only way nutrient enters the living system. Each tick's uptake is split immediately by **kappa**, mirroring the energy surplus split (flow 9): the (1 − kappa) share is earmarked as **reproductive nutrient** (off-limits to growth, reserved for reproduction), the kappa share added to the free store growth can bind. Unlike energy, nutrient has no retention buffer — there is no per-tick nutrient cost to hold reserve against, so the split applies to the whole uptake flow.

The expected property that mobile agents cannot efficiently extract nutrients emerges from the cost structure, not from a separate gating mechanism. In terrestrial ecology, nutrient extraction requires physical infrastructure embedded in the substrate — roots, hyphae, membrane surfaces — that is fundamentally incompatible with mobility (Lambers, Chapin & Pons 2008). In the world physics, this incompatibility is expressed through superlinear maintenance costs: investing in both autotrophy and mobility is prohibitively expensive, and since nutrient uptake is implicit in autotrophy, an agent that pays for mobility is not paying for the infrastructure needed to extract nutrients. If genesis search reveals that the cost curve is not steep enough to prevent mobile autotrophy from being viable, the remedy is to steepen the autotrophy-mobility interaction cost, not to add a temporal gate. The available pool is spatially heterogeneous and shared among co-located agents, creating local competition for nutrient independent of competition for light.
...
> **Status:** Not yet implemented — see [#249](https://github.com/samthorold/explorers/issues/249).
...
**2. Sessile vs. mobile.** Sessile and mobile strategies face fundamentally different resource access. The sessile strategy dominates for autotrophs because sunlight is ambient — mobility costs energy for zero additional energy income. A mobile autotroph pays locomotion costs but does not reach more light than a stationary one. The divide is reinforced by superlinear maintenance costs: investing in both autotrophy and mobility is prohibitively expensive, and since nutrient uptake is implicit in autotrophy, a mobile agent paying for mobility overhead is not investing in the infrastructure needed to extract nutrients. No explicit gate prevents mobile agents from photosynthesizing — the economics make it unviable. If genesis search finds viable mobile autotrophs, the autotrophy-mobility cost interaction should be steepened.
