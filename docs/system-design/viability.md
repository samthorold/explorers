...
- **Dynamics failures — out of cheap a priori reach (for now).** *Monoculture* vs. coexistence, *frozen dynamics* vs. oscillation, *population explosion*, and *generalist dominance* are not about whether a fixed point exists but about its *stability and multiplicity* — bifurcation and stability territory. (Generalist dominance was once filed as a candidate existence gate; the analysis below shows its prevention rests on structural fragility and functional incompatibility — a mortality-and-morphology force, not a static income/cost inequality — so it belongs here.) They remain the domain of the search until the affirmative theory is built out.
