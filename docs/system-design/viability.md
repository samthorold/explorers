...
**Caveat — confounded by a mobile-feeding defect; pending re-validation (#379).** The confirmation above was read from runs predating the fix for #379, in which a contact-duration ramp on consumption (since removed; see [world rules](world-rules.md#consumption-is-incremental-and-its-character-emerges-from-target-and-size)) reset on every movement and so left *any* mobile consumer unable to feed at all. In `example12` the broad generalists and the specialist mobile consumers are mobile (0.5 and 0.3 respectively), while the surviving compatible mixotrophs are sessile (0.0) — the very axis of the in-run control. Their elimination in those runs is therefore partly attributable to **starvation from the defect** rather than to fragility and incompatibility alone: the control was confounded with broken feeding, measuring mobility's effect on feeding as much as the cross-trait incompatibility the reading attributes it to. The *prediction* (fragility + incompatibility confine generalists without an interaction term) is not withdrawn — but its empirical support must be re-established by re-running `example12` once mobile consumers can feed. Until then, treat #325 as a prediction with provisional, confounded support, not a clean confirmation.
