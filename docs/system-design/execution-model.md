...
7. **Check death thresholds** — agents that starved this tick (metabolism emptied their reserve; a later phase crediting them before the check does not revive them) or whose structure has dropped below their complexity-dependent death threshold die, producing carcasses. (Runs last, after wear, so that a tick's own movement-wear can carry an agent over its death threshold within the same tick.)
