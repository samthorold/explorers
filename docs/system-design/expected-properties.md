...
The descriptor for this property, `coexistence_duration`, is the fraction of sampled ticks in the settled window `(T/2, T]` ([genesis search](genesis-search.md), *One rollout, one settled window*) whose living population resolves into ≥2 trait-space clusters (DBSCAN over the trait vectors at each sampled tick) — a *persistence* reading. It is distinct from `clustering_strength`, which is an *existence* reading: whether the single final snapshot is multimodal (dip/valley-depth). The two answer different questions — *does* multimodal structure exist, versus *how long* ≥2 clusters co-occur — so they can legitimately disagree.

**The zero floor is a valid reading, not a defect.** `coexistence_duration = 0` — no sampled settled-window tick ever carries ≥2 clusters — is technically possible even where `clustering_strength` is high. A world whose modes appear only transiently, or only in the final frame, reads as clustered yet never-coexisting; a generalist monoculture reads as a single cluster throughout. A flat zero beside visible final-snapshot structure looks nonsensical, but it is the faithful measurement of a world that does not actually hold multiple niches open over time — a non-sensible world on the coexistence↔monoculture axis. This axis is *dynamics*, not an a priori dead-zone (see [viability](viability.md) — monoculture↔coexistence is "out of cheap a priori reach"), so the search does **not** gate on it; it deprioritises such worlds through the `0.2 · coexistence_duration` term in the fitness composition. They are selected out, not flagged as errors.
...
**What the detector measures.** The evaluator's `generalist_dominance` gate reads each living agent's *own* trophic coordinates (autotrophy and heterotrophy shares): an agent is a generalist when both exceed `generalist_threshold`, and the gate fires when generalists hold more than `generalist_dominance_fraction` of living energy. It is read per agent, never off trait-space cluster means (#486): a DBSCAN cluster that merges pure producers with pure heterotrophs has a mean that sits above the threshold on both axes, yet holds no generalist at all. The trophic balance term is scored the same way — each agent bucketed by the topology's trophic-role read (producer when autotrophy ≥ heterotrophy), not each cluster by its mean — so a merged cluster scores its producer energy share rather than a whole-cluster 0 or 1 decided by a producer/consumer count tie.
