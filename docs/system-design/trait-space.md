...
This keeps decomposition behavioural even though a sessile saprotroph has a viable home. Feeding reach already has a sessile solution — the body-extent term in `consumption_reach`, the mycelium-as-foraging-organ channel, so a growing high-heterotrophy agent reaches carcasses a smaller one cannot (see [world-rules](world-rules.md), flow 3). That mechanism extends *every* heterotroph's reach with structure; it does not make "decomposer" heritable — a sessile saprotroph still occupies the same trait-space point a sessile consumer-of-the-living would. The live question is therefore not viability but whether a detrital guild reliably *forms* under genesis when nothing but behaviour distinguishes it from a predator the search can select on. That is an empirical question for genesis trophic-balance evaluation on correctly-specified worlds — not a reason to add a decomposer axis.
