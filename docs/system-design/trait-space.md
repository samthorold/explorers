...
## Decomposer is a behavioural role, not a heritable trait

There is no decomposer dimension, and the current vector cannot hold one even if we wanted it. A carcass retains the *exact* trait vector of the agent that died — a dead producer and a living producer occupy the same point in trait space. Because trophic transfer efficiency is governed by trait-space distance (see [world-rules](world-rules.md)), an agent efficient at draining a producer-carcass is *equally* efficient at draining the living producer beside it: trait distance cannot separate predation from decomposition. "Decomposer" is therefore a reading of what an agent eats — the network position it occupies, in the sense of [trophic roles](../ecology/trophic-roles.md) — never a cluster you can point to in the trait vector. This is the same one-capability stance as the heterotrophy dimension above: target state decides the role, not separate machinery.

This is a deliberate simplification, and it has a known edge worth naming. The one heritable signature that *could* mark out a decomposer guild is mobility — a sessile saprotroph against a mobile predator. But mobility alone does not make a pure decomposer viable: a mobility-0 agent is a dimensionless point with a fixed contact radius and a scalar structure, so it mines out its home nutrient cell and starves beside carcass nutrient it cannot reach. It has no analogue of the mycelium (body-as-reach), extracellular digestion, or spore dispersal that make real saprotrophs viable. So the brown/detrital pathway — the dominant energy flow in many real ecosystems — currently runs through *mobile* detritivores read behaviourally, not through trait-distinct sessile decomposers. Whether the design eventually needs a body-as-reach mechanism or a carcass-preference axis to give sessile saprotrophy its own niche is an open tension; the present design reads decomposition behaviourally from the energy and event flow and holds the vector at seven dimensions.
