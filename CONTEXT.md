...
**Heterotroph**:
A per-individual role tag, read by trait: an agent whose **heterotrophy** exceeds its autotrophy (any other agent reads as a producer). Among heterotrophs, **consumer** versus **decomposer** is read from realised diet — the share of consumed energy drained from carcasses — with a non-eater defaulting to consumer. The tag says what one agent *is*; it says nothing about whether there is a population of them. A single sessile, sterile mixotroph reads as a heterotroph exactly as a breeding cohort of twenty does — the population-level term is the **heterotroph guild**.
_Avoid_: trophic level, second trophic level (those name a population, which the tag does not establish), predator (consumption is non-lethal by default)

**Heterotroph guild**:
A heterotroph *population* of one role — consumer or decomposer — as distinct from a heterotroph *individual* carrying the role tag. A guild holds over a run when the role's count reaches a minimum size on **every** sampled tick of the run's second half *and* at least one birth in that window has a guild member as a parent (sustained size plus recruitment), which rules out both a lone long-lived individual and a sterile founder cohort sitting at the floor. Read by the evaluator for both heterotroph roles and reported, per seed, on the **atlas** as a fraction of the cell's ensemble — under the same authority boundary as the **coexistence fraction**: never a **behaviour axis**, never a fitness term. The minimum size and the window are starting values; the atlas is what tests them.
_Avoid_: decomposer presence, has-a-decomposer (a role tag on one agent is exactly what a guild is not), trophic level (the guild is the observable that would justify that word)
...
**Atlas**:
The output of world genesis — a map from world-parameter space onto the failure-mode coordinates. It pairs an **archive** of the best surviving worlds found, each binned by its position on the three **behaviour axes**, with a **dead frontier** recording which regions fail and on which cliff. The atlas is the deliverable: it shows the *structure* of viability — many viable worlds across a manifold, not one tuned point. A single **world recipe** is one projection of it (the highest-scoring cell that is robustly sensible across initial conditions — most of its ensemble coexists), but the map's value is its plurality. Each cell also carries reported per-seed distributions across its ensemble — the **heterotroph guild** fractions (decomposer and consumer) and the **coexistence fraction** — never folded into the cell's score.
...
**Permanence**:
The formal notion of a non-collapsing system: the extinction boundary — the set of states where a compartment (producers, consumers, living nutrient) is empty — is a *repeller*, so every trajectory that starts inside stays bounded away from it. Cycles and chaos are permitted; only reaching the boundary is forbidden. It is the affirmative counterpart of the **degenerate configurations**: where a viability gate says a region *cannot* hold life, a permanence condition says the living state *persists*. In this project it is proven on the mean-field reduction of the world rules (the producer-invasion ratio, the consumer-invasion number, and the lockup repeller) and held as a characterisation of the individual-based model, never a per-seed promise — demographic chance and spatial reach can collapse a world the reduction calls permanent.
_Avoid_: stability, equilibrium (both imply a fixed point, which oscillation violates)

**World recipe**:
A playable world drawn from the **atlas** — the elite of one of its cells. A combination of **world parameters**, an **initial distribution**, and a **max ticks** count — the minimal specification needed to deterministically create a world given a seed. Does not contain a seed (each playthrough generates a fresh one). Max ticks is the tick count at which genesis certified the ecology as sensible — the app fast-forwards to this point before the player enters. The simulation is deterministic: the same recipe + seed always produces the same world history. The default pick is the highest-scoring cell that is robustly sensible across initial conditions (most of its ensemble coexists), falling back to plain highest-scoring when none clears that bar; but any viable cell yields a recipe — the recipe is a projection of the atlas, not the atlas's purpose.
